...
last_cs = css.select { |cs| ! cs.timestamp.nil? && cs.timestamp <= last_timestamp && cs.type != :unknown }.max_by { |cs| cs.timestamp }
...
# delay_to_apply_in_ms = delay - first_changes_timestamp
# #delay_to_apply_in_ms = delay - (first_changes_timestamp + 3600000)


# video_path = 'VIDEOS_PROCESSING'
# video_file = '20130528-1022-G17-EXP2-USER3-screencast.webm'
# 
# video_start_time_ms = 56000
# video_start_time = Time.at(video_start_time_ms / 1000, video_start_time_ms % 1000 * 1000).strftime("%M:%S.%L")
# puts video_start_time
# 
# expected_duration_in_s = 6
# video_end_time_ms = expected_duration_in_s * 60 * 1000
# video_end_time = Time.at(video_end_time_ms / 1000, video_end_time_ms % 1000 * 1000).strftime("%M:%S.%L")
# puts video_end_time

# # mencoder --ss [[hh:]mm:]ss[.ms] endpos [[hh:]mm:]ss[.ms]
# puts "mencoder -oac copy -ovc copy -ss #{video_start_time} -endpos #{video_end_time} -o repack-#{video_file} #{video_path}/#{video_file}"



# ffmpeg -i test.webm -c:v libx264 -preset slow -crf 22 -c:a copy output.mkv
