...
def slice_chat(num, experiment_name):
	...
	x_min_after_end_of_audio_revs = [ get_revision_at_time(revisions, end_of_audio_rev['datetime'] + timedelta(minutes=x)) for x in xrange(1,6)]

	i = 0
	for rev in x_min_after_end_of_audio_revs:
		i = i + 1
		print "== %i min. after end of audio version - rev: %s (%s)" % (i, rev['rev'], format_time(rev['datetime']))
	...
	print_chat_entries(selected)
	write_chat_word_counts_as_csv(OUTPUT_DATA_PATH + experiment_name + num + '-before-end-audio.csv', selected)
	...
	for rev in x_min_after_end_of_audio_revs:
		...
		print_chat_entries(selected)
		write_chat_word_counts_as_csv(OUTPUT_DATA_PATH + experiment_name + num + '-before-%imin-after-end-audio.csv' % i, selected)		
		print '\t-- %i min after end of audio --' % (i)

	print_chat_entries(entries)
	write_chat_word_counts_as_csv(OUTPUT_DATA_PATH + experiment_name + num + '-after-5min-after-end-audio.csv', entries)		
